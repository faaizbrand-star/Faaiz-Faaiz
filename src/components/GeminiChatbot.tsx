...
${brandName.toUpperCase()} AI MARKET ASSISTANT — CHAT TRANSCRIPT
Role: ${ROLES[selectedRole].name} (${ROLES[selectedRole].shortDesc})
...
${divider}\n\n`;
    ...
    const body = messages
      .map((msg, index) => {
        ...
        const sender = isUser ? 'YOU (TRADER)' : `${brandName.toUpperCase()} AI [${msg.modelUsed || modelOverride}]`;
        const sentiment = !isUser ? detectMarketSentiment(msg.text) : null;
        const sentimentInfo = sentiment ? ` | Market Sentiment: ${sentiment}` : '';

        return `[#${index + 1}] ${sender} • ${msg.timestamp}${sentimentInfo}\n${subDivider}\n${msg.text}\n`;
      })
      .join('\n\n');

    const footer = `\n\n${divider}
END OF CONVERSATION TRANSCRIPT
Generated by ${brandName} AI Market Assistant
${divider}`;
    ...
    link.href = url;
    link.download = `faaiz-ai-chat-${dateStr}.txt`;
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 bg-[#123D32] border-b border-[#F2D231]/20">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {/* Export Chat as Plain Text File Button */}
            <button
              id="chat-export-history-btn"
              type="button"
              onClick={handleExportConversation}
              disabled={messages.length === 0}
              title={messages.length === 0 ? "No messages to export" : "Export current conversation as a plain text file (.txt)"}
              aria-label="Export chat conversation as plain text file"
              className="p-1.5 sm:px-2.5 sm:py-1 rounded-lg text-xs font-spacemono flex items-center gap-1.5 text-gray-300 hover:text-white hover:bg-white/10 border border-white/10 hover:border-[#F2D231]/40 transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
            >
              {isExported ? (
                <>
                  <Check className="w-3.5 h-3.5 text-emerald-400" />
                  <span className="hidden sm:inline text-emerald-400 font-semibold">Saved .txt</span>
                </>
              ) : (
                <>
                  <Download className="w-3.5 h-3.5 text-[#F2D231]" />
